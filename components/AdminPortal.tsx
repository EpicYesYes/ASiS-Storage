...
const AdminPortal: React.FC<AdminPortalProps> = ({ 
  ...
  const handleAddStudentSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Fix: Ensure a space if separator is empty or (none)
  ...
  const handleTeacherSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // Fix: Maintain space if (none) is used
  ...
  const renderTeacherForm = (isNew: boolean) => (
    <div className="bg-asis-card p-10 rounded-[3rem] border border-asis-border shadow-xl space-y-8 animate-in slide-in-from-top-4">
      <div className="flex items-center justify-between">
        <h3 className="text-2xl font-black">{isNew ? 'Daftar Guru Baharu' : `Profil Guru`}</h3>
      ...
      <form onSubmit={handleTeacherSubmit} className="space-y-8">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-asis-border">
          <button type="button" onClick={() => { setShowAddTeacher(false); setEditingTeacherId(null); }} className="px-8 py-4 font-black opacity-40 uppercase text-xs">Batal</button>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 text-asis-text transition-colors duration-300">
      ...
      <div className="bg-asis-card p-8 rounded-[2.5rem] border border-asis-border shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 transition-colors">
        <div><h2 className="text-3xl font-black tracking-tight">{t('adm_title')}</h2><p className="opacity-60 mt-1 font-black italic uppercase text-[10px] tracking-widest">Pengurusan Sekolah ASiS</p></div>
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4">
           ...
           <div className="bg-asis-card p-10 rounded-[3rem] border border-asis-border shadow-xl">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
               <div><h3 className="text-2xl font-black">{t('adm_teacher_dir')}</h3><p className="text-xs opacity-40 font-black uppercase tracking-widest">Urus akses staf</p></div>
