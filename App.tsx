...
const App: React.FC = () => {
  ...
  }, [students, teachers, records, cases, currentUser, isLoaded]);
      ...
      <main className="flex-1 overflow-y-auto max-h-screen">
        <header className="sticky top-0 z-30 bg-asis-bg/80 backdrop-blur-md border-b border-asis-border px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {syncStatusMsg && <div className="text-[10px] font-black uppercase bg-asis-card px-4 py-2 rounded-xl">{syncStatusMsg}</div>}
            <button onClick={handleCloudPull} className="p-2.5 rounded-xl border border-asis-border bg-asis-card hover:bg-asis-primary transition-all">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </button>
            <button onClick={() => setActiveView('account')} className="w-10 h-10 rounded-xl bg-asis-primary flex items-center justify-center font-black">{currentUser.name[0]}</button>
          </div>
        </header>
        <div className="p-8">
          ...
          {activeView === 'student-detail' && <StudentDetail student={studentWithPoints.find(s => s.id === selectedStudentId)!} teacher={currentUser} onBack={() => setActiveView('students')} onUpdate={addRecord} onUpdateAvatar={() => {}} onRemove={() => {}} t={t} />}
          {activeView === 'admin' && (
            <AdminPortal 
